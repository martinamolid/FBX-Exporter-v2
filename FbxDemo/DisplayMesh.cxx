...
void DisplayControlsPoints(FbxMesh* pMesh)
{
    ...
    for (i = 0; i < lControlPointsCount; i++)
    {
		...
		//file << "        Control Point " << i << endl;
        Display3DVector("            Coordinates: ", lControlPoints[i]);
		//file << "            Coordinates: " << *lControlPoints[i] << " " << endl;
		//file << Print3DVector("            Coordinates: ", lControlPoints[i]);
        ...
        for (int j = 0; j < pMesh->GetElementNormalCount(); j++)
        {
			...
			if (leNormals->GetMappingMode() == FbxGeometryElement::eByControlPoint)
			{
				...
				if (leNormals->GetReferenceMode() == FbxGeometryElement::eDirect)
					...
					//file << Print3DVector(header, leNormals->GetDirectArray().GetAt(i));
			}
        }
    }
...
{
	//ofstream file;
	//file.open("testing.txt", ios::out | ios::app);
    ...
    for (i = 0; i < lPolygonCount; i++)
    {
		...
		for (j = 0; j < lPolygonSize; j++)
		{
			...
			else 
			{
				...
				//file << Print3DVector("            Coordinates: ", lControlPoints[lControlPointIndex]);
			}
